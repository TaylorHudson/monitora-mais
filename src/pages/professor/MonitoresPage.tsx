...
import { Eye, SearchX, UserMinus2 } from 'lucide-react';
import { Button } from '../../components/ui/button';
import {
  converterDiaParaPortugues,
  formatarDias,
  formatarHora,
} from '../../utils/utils';
...
export default function MonitoresPage() {
  ...
  async function carregarAgendamentosAprovados(
    monitor: Monitor,
    monitoria: MonitoriaDetails,
  ) {
    try {
  ...
  const monitoriasFiltradas = monitorias
    .filter((m) => {
      if (!filtro) return true;
      return m.name.toLowerCase().includes(filtro.toLowerCase());
    })
    .flatMap((monitoria) =>
      monitoria.students.map((monitor) => ({
        monitoria,
        monitor,
      })),
    );
      ...
      <Dialog open={verDetalhesAberto} onOpenChange={setVerDetalhesAberto}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Monitorias aprovadas para este monitor nesta disciplina
            </DialogDescription>
          </DialogHeader>
              ...
              {agendamentosAprovados.map((item, index) => (
                <TableRow key={index}>
                  <TableCell className="max-w-[180px] truncate text-gray-700">
                    <span title={item.discipline}>{item.discipline}</span>
                  </TableCell>
          ...
          <main className="flex-1 p-4 md:p-8 min-h-screen">
            <div className="mb-8 text-left w-full">
              ...
              <p className="text-gray-700 text-base text-left">
                Gerencie os monitores de suas disciplinas
              </p>
            ...
            {monitoriasFiltradas.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-[60vh] text-center">
                ...
                <p className="text-gray-500 mt-2">
                  Não encontramos monitores para esse filtro
                </p>
              </div>
            ) : (
              <Table className="mt-6 bg-white/70 rounded-xl shadow-sm">
                ...
                <TableBody>
                  {monitoriasFiltradas.map(({ monitoria, monitor }) => (
                    <TableRow
                      key={`${monitoria.id}-${monitor.registration}`}
                      className="hover:bg-primary/5 transition"
                    >
                      <TableCell className="max-w-[200px] truncate text-gray-700">
                        <span title={monitoria.name}>{monitoria.name}</span>
                      </TableCell>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription className="text-gray-600">
                                  Ao cancelar a inscrição deste monitor, ele
                                  perderá imediatamente o acesso ao sistema.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
